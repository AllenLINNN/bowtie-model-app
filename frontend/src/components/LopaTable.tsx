    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/70 dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 backdrop-blur-md shadow-sm rounded-xl p-6 transition-all duration-300">
        ...
        <div className="flex items-center gap-3 self-stretch md:self-auto flex-wrap">
          ...
          <button
            onClick={exportToCSV}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 shrink-0"
          >
            <Download size={14} />
            導出 CSV 報表
          </button>
      ...
      {scenarioPaths.length === 0 ? (
        <div className="bg-white/70 dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 rounded-xl p-12 text-center backdrop-blur-md">
          ...
          <button
            ...
            className="mt-6 inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 text-sm font-semibold px-4 py-2 rounded-lg transition-all"
          >
            <Compass size={14} />
            前往畫布編輯
          </button>
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 rounded-xl shadow-md overflow-hidden backdrop-blur-md transition-all duration-300">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-800/40 text-gray-600 dark:text-gray-300 text-base font-bold border-b border-gray-200 dark:border-slate-800">
              ...
              <tbody className="divide-y divide-gray-150 dark:divide-slate-800 text-base">
                {scenarioPaths.map((path, idx) => {
                            ...
                            >
                              <div className="flex justify-between items-start gap-2">
                                <div className="flex flex-col gap-0.5 min-w-0">
                                  ...
                                  <span className={`inline-block self-start text-[11px] px-1 py-0.5 rounded font-bold ${badgeBg}`}>
                                    {badgeLabel}
                                  </span>
                                </div>
                              ...
                              {isExcludedInInitial && (
                                <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-0.5 mt-1">
                                  <Info size={12} />
                  ...
                  return (
                    ...
                    >
                      {/* 編號 */}
                      <td className="py-4 px-4 font-mono font-medium text-gray-500 dark:text-gray-400 text-base">
                        {idx + 1}
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <span className="text-gray-450 dark:text-gray-600 text-[15px]">➔</span>
                            <span className="bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-300 px-1.5 py-0.5 rounded font-bold text-[15px]">
                              {cCode}
                            </span>
                          </div>
                          <div className="text-[15px] text-gray-500 dark:text-gray-450 mt-1 line-clamp-2 leading-relaxed">
                            <strong className="text-gray-800 dark:text-gray-200 text-[15px]">{threatNode?.data?.label || '未命名威脅'}</strong>
                            <br />
                            導致 ➔ <span className="italic text-gray-700 dark:text-gray-300">{consequenceNode?.data?.label || '未命名後果'}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 font-medium text-gray-700 dark:text-gray-300">
                        {path.initiating_event.input_mode === 'semi_quantitative' ? (
                          <div className="flex flex-col">
                            <span className="font-bold text-base">L-{path.initiating_event.semi_quant_level}</span>
                            <span className="text-sm text-gray-450 dark:text-gray-500 mt-0.5">
                              (半定量模式)
                            </span>
                          </div>
                        ) : (
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-sm text-gray-450 dark:text-gray-500">
                              次/年
                            </span>
                          </div>
                        )}
                      ...
                      {/* TMEL 目標 */}
                      <td className="py-4 px-4 text-right font-mono font-medium text-gray-650 dark:text-gray-300 text-base">
                        {targetTmel ? targetTmel.toExponential(1) : '無限制'}
                      </td>

                      {/* 最終頻率 */}
                      <td className="py-4 px-4 text-right font-mono font-bold text-gray-800 dark:text-gray-200 relative group cursor-help text-base">
                        <div className="flex flex-col items-end gap-0.5">
                          <span>{currentFreq.toExponential(2)}</span>
                          <span className="text-sm text-blue-500 dark:text-blue-400 border-b border-dashed border-blue-300 dark:border-blue-700">
                            檢視公式
                          </span>
                        </div>
                      ...
                      <td className="py-4 px-4 text-center">
                        {isMeetsTarget ? (
                          <div className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 rounded-full font-bold text-base">
                            <CheckCircle size={16} />
                            <span>合規</span>
                          </div>
                        ) : (
                          <div className="flex flex-col items-center gap-1">
                            <div className="inline-flex items-center gap-1 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 px-2.5 py-1 rounded-full font-bold text-base">
                              <XCircle size={16} />
                              <span>超標</span>
                            </div>
                            {currentRiskGap && (
                              <span className="text-[15px] text-red-500 dark:text-red-400 font-extrabold">
                      ...
                      <td className="py-4 px-4 text-center">
                        <button
                          ...
                          className="inline-flex items-center gap-1 bg-gray-100 hover:bg-blue-50 dark:bg-slate-800 dark:hover:bg-blue-950/30 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700/60 transition-all font-semibold text-sm"
                          title="跳轉定位到畫布編輯此場景"
                        >
                          <Compass size={14} />
                          <span>跳轉定位</span>
